    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        if (dist < flakeSize) {
          ...
          // Solid white where visible, soft fade to fully transparent otherwise —
          // avoids the translucent mid-gray flakes that read as "confetti/ash".
          float a = smoothstep(0.12, 0.45, intensity);
...
export default function PixelSnow({
  ...
  useEffect(() => {
    ...
    // The container may have zero height at mount (before layout settles).
    // Observe it and resize the renderer once it has real dimensions.
    const ro = new ResizeObserver(() => {
      ...
      if (w > 0 && h > 0) {
        renderer.setSize(w, h);
        material.uniforms.uResolution.value.set(w, h);
        // Redraw immediately so the reallocated buffer never shows a blank frame
        // while the hero grows during the chat-open transition.
